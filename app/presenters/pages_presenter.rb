...
private
  ...
  def page_text(base_path)
    ...
    raw(content_tag(:a, title, href: base_path, class: "display-block")) +
      raw(content_tag(:span, base_path, class: "govuk-body-s display-block"))
  ...
  def top_phrases(page)
    ...
    # rubocop:disable Rails/OutputSafety
    raw(phrases.map { |phrase| content_tag(:span, phrase, class: "phrase") }.join(""))
    # rubocop:enable Rails/OutputSafety
  end
end
